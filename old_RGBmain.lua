...
local opt = lapp[[
   ...
   -m,--model         (default "convnet")   type of model tor train: convnet | mlp | linear | vggStyle_01
...
for j=1,trainset.data:size(2)*#channels do  -- #frames * #channels
  mean[j] = trainset.data[{{},j,{},{}}]:mean()
  std[j] = trainset.data[{{},j,{},{}}]:std()
  trainset.data[{{},j,{},{}}]:add(-mean[j])
  trainset.data[{{},j,{},{}}]:div(std[j])
end

torch.save('normRGB_trainset.t7',trainset)

channels = {'r','g','b'}

for j=1,testset.data:size(2)*#channels do  -- #frames * #channels
  testset.data[{{},j,{},{}}]:add(-mean[j])
  trainset.data[{{},j,{},{}}]:div(std[j])
end
