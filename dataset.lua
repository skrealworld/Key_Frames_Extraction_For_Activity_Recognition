...
function dataset:__init(...)
   ...
   local extensionList = {'mat','mat'}
   local findOptions = ' -iname "*_Y_**.' .. extensionList[1] .. '"'
  ...
  --    findOptions = findOptions .. ' -o -iname "*.' .. extensionList[i] .. '"'
   ...
   for i=1,#self.classes do
      ...
      if length == 0 then
	 print('classFindFiles[i]' .. classFindFiles[i])
         print('self.classes:- ' .. self.classes[i])
...
function dataset:getByClass(class)
   ...
   local imgpathY = ffi.string(torch.data(self.imagePath[self.classListSample[class][index]]))
   --print("Imagepath before string sub",imgpathY) 
   imgpathCbCr=string.gsub(imgpathY,"_Y_","_CbCr_")
   imgpathOF=string.gsub(imgpathY,"_Y_","_OF_")
   --print("imrrg after gsuv=b",imgpathY)
   temp_out.Y=self:sampleHookTrain(imgpathY)
   temp_out.CbCr=self:sampleHookTrain(imgpathCbCr)
   temp_out.OF=self:sampleHookTrain(imgpathOF)
   --print("RETURN IN GETBY CLASS", temp_out)
   return temp_out
...
local function tableToOutput(self, dataTable, scalarTable)
   ...
   --print('size of datatable ',#dataTable)
   
   assert(dataTable[1]:dim() == 3)
   --print("DATATABLE", dataTable[1]:size(1))
   if dataTable[1]:size(1)==10 then
   data = torch.Tensor(quantity,dataTable[1]:size(1),dataTable[1]:size(2),dataTable[1]:size(3))
   end
   if dataTable[1]:size(1)==20 then
   data = torch.Tensor(quantity,dataTable[1]:size(1),dataTable[1]:size(2),dataTable[1]:size(3))
   end
   if dataTable[1]:size(1)==3 then 
   data = torch.Tensor(quantity,dataTable[1]:size(1),dataTable[1]:size(2),dataTable[1]:size(3))
   end
   --print(#data)
   --print(dataTable[1])
   ...
   for i=1,#dataTable do
     -- print('size of data[i]', #data[i])
      --print('size of dataTable[i]',#dataTable[i])      
...
function dataset:sample(quantity)
  --print('check-1',quantity)
   assert(quantity)
   local Y_dataTable={}
   local CbCr_dataTable={} 
   local OF_dataTable={}
   --local dataTable = {}
   local scalarTable = {}
   for i=1,quantity do
...
--      print(class)
    ...
    --  print(out)
      table.insert(Y_dataTable,out.Y)
      table.insert(CbCr_dataTable,out.CbCr)
      table.insert(OF_dataTable,out.OF)
      table.insert(scalarTable, class)
   end
   --print(dataTable)
   
   local Y_data, scalarLabels = tableToOutput(self, Y_dataTable, scalarTable)
   local CbCr_data,_ = tableToOutput(self, CbCr_dataTable, scalarTable)
   local OF_data,_=tableToOutput(self, OF_dataTable,scalarTable)
   --print('Y_data',#Y_data)
   --print('CbCr_data',#CbCr_data)
   data = {Y_data,CbCr_data,OF_data}
   --print("DATA IN SAMPLE",data)
   return data, scalarLabels
end

os.execute("rm Input_label.txt")
os.execute("rm Input_label_CbCr.txt")
function dataset:get(i1, i2)
   filef=io.open("Input_label.txt","aw")
   fileCbCr=io.open("Input_label_CbCr.txt","aw")
   ...
   for i=1,quantity do
      -- load the sample
      local imgpathY = ffi.string(torch.data(self.imagePath[indices[i]]))
      local outY = self:sampleHookTest(imgpathY)
      local imgpathCbCr=string.gsub(imgpathY,"_Y_","_CbCr_")
      local outCbCr=self:sampleHookTest(imgpathCbCr)
      local imgpathOF=string.gsub(imgpathY,"_Y_","_OF_")
      local outOF = self:sampleHookTest(imgpathOF)
      
      table.insert(dataTableY, outY)
      table.insert(dataTableCbCr,outCbCr)
      table.insert(dataTableOF,outOF)
      table.insert(scalarTable, self.imageClass[indices[i]])
      filef:write(imgpathOF.."  "..self.imageClass[indices[i]])
      fileCbCr:write(imgpathCbCr.."  "..self.imageClass[indices[i]])
      filef:write("\n")
      fileCbCr:write('\n')
      end
   
   --dataTable={Y_dataTable,CbCr_dataTable}
   local dataY, scalarLabels = tableToOutput(self, dataTableY, scalarTable)
   local dataCbCr,_=tableToOutput(self, dataTableCbCr,scalarTable)
   local dataOF,_=tableToOutput(self, dataTableOF,scalarTable)
   data={dataY,dataCbCr,dataOF}
   return data, scalarLabels
end
